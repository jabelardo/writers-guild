 ...
 */
export function canonicalStringify(obj, allowedKeys = null, seen = []) {
  ...
  const seenIndex = seen.push(obj) - 1;
  try {
    if (Array.isArray(obj)) {
      return '[' + obj.map((v) => canonicalStringify(v, allowedKeys, seen)).join(',') + ']';
    }
    ...
    for (const key of keys) {
      ...
      pairs.push(JSON.stringify(key) + ':' + canonicalStringify(value, allowedKeys, seen));
    }
    return '{' + pairs.join(',') + '}';
  } finally {
...
function sortedEntries(entries, allowedEntryKeys) {
  ...
  const withKey = entries.map((entry) => {
    ...
    return { entry, sortKey: JSON.stringify(keys.toSorted()) };
  });
...
/** Keys inside `extensions` that are included. */
const CHAR_EXT_ALLOWLIST = new Set(['depth_prompt', 'talkativeness', 'fav', 'avatar']);
...
export function computeLorebookChecksum(lorebookData) {
  ...
  if (lorebookData.extensions && typeof lorebookData.extensions === 'object') {
    const keys = Object.keys(lorebookData.extensions).toSorted();
    const filtered = {};
    for (const key of keys) {
